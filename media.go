...
func Source(li *Lispy) string {
	const htmlstr = `<source{{if exist "src"}} src="{{getdel "src"}}"{{end}}{{if exist "media"}} media="{{getdel "media"}}"{{end}}{{if exist "type"}} type="{{getdel "type"}}"{{end}}{{range names}} {{.|attr}}="{{get .}}"{{end}} />`
...
func Track(li *Lispy) string {
	const htmlstr = `<track{{if exist "default"}} default="{{getdel "default"}}"{{end}}{{if exist "kind"}} kind="{{getdel "kind"}}"{{end}}{{if exist "label"}} label="{{getdel "label"}}"{{end}}{{if exist "src"}} src="{{getdel "src"}}"{{end}}{{if exist "srclang"}} srclang="{{getdel "srclang"}}"{{end}}{{range names}} {{.|attr}}="{{get .}}"{{end}} />`
